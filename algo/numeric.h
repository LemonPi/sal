...
make_cyclic(base, prime)    -> makes a cyclic number from 1/prime in the base (ex. decimal base
10)
...
class Scale {
   public:
    ...
    OutType operator()(InType in) { return (_rangeOut * (in - _minIn) / _rangeIn) + _minOut; }
...
struct mul_impl {
    static Item mul(const Indexable& items) {
        Item res{items[0]};
        for (size_t i = 1; i != items.size(); ++i) res *= items[i];
        return res;
    }
};
...
struct mul_impl<Matrix<T>, Indexable> {
    static Matrix<T> mul(const Indexable& mats) {
        ...
        for (size_t trivial = 0; trivial < n; ++trivial) min[trivial][trivial] = 0;
    ...
    static Matrix<T> mul_trace(const Indexable& mats, const Splits& split_at, size_t start,
                               size_t end) {
        if (start == end) return mats[start];
        if (end - start == 1)
            return mats[start] * mats[end];
        else
            return mul_trace(mats, split_at, start, split_at[start * mats.size() + end]) *
...
std::vector<T> factorize(T num) {  // works great for smooth numbers (small factors)
    ...
    T d{3};
    bool first_factors = true;
    while (num >= d * d || first_factors) {
...
std::vector<T> factorize_rough(T num) {
    ...
    for (size_t i = 1; primes[i] <= max_prime;) {
        ...
        ++i;
        if ((i == primes.size() || primes[i] > max_prime) && num > 1) {
            factors.push_back(num);
            return factors;
        }
    }
...
// occurs in counting problems like number of possible BST with n unique keys, valid parenthesis
// pairing counting
...
size_t catalan(size_t n) {
    size_t res = 1;  // C_0 = 1
    for (size_t i = 1; i < n; ++i) res = (2 * (2 * i + 1) * res) / (i + 2);
    return res;
}
}