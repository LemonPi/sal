...
enum Function_index {
    ...
    INTERSECTION,
    PERM, ALLPERMS,
    FACTORIZE, FACTORIZE_ROUGH,
    TOTIENT,
    GCD,
    LCM,
    FIBONACCI,
    INF_PRIME, NTH_PRIME, IS_PRIME, COUNT_PRIME, NEXT_PRIME, CLOSEST_PRIME,
    BUB_SORT, CNT_SORT, INS_SORT, HEAP_SORT, LIN_SORT, MER_SORT, PAT_SORT, QCK_SORT, RDX_SORT, TIM_SORT, SORT,
    LEVENSHTEIN,
    SA_LC_SUBSTRING, LC_SUBSTRING,
    LC_SUBSEQ, LC_SUBSEQ_LEN,
    EXPERIMENT
};
...
const map<string, Function_index> algonames {
    {"0", RANDGEN},
    {"1", SHUFGEN},
    {"select", SELECT},
...
vector<vector<T>> data_store(const string& fname, Op vlist_gen, int l_num = list_num, int l_size = list_size, int r = range) {
    ofstream f {fname};
...
int algotest(std::vector<std::string> args) {
    ...
    switch (algo_index) {
        ...
        case RANDGEN:
        case SHUFGEN:   
            if (args.size() < 4) out << "Random generation needs l_num l_size range as argument\n";
            else {
                list_num = tol(args[1]);
                list_size = tol(args[2]);
                range = tol(args[3]);
                if (name[0] == '0') data_store<int>(fname, rand_vlist<int>, list_num, list_size, range);
                if (name[0] == '1') data_store<int>(fname, shuffled_vlist<int>, list_num, list_size, range);
            }
            break;
        case SELECT: {
            if (args.size() < 4) out << "Number sequence needs to be longer and followed by an index (1 = smallest)\n";
            else {
                vector<long> nums;
        ...
        case BIN_SEARCH: {
            if (args.size() < 4) { load_data(vlist); time.restart(); for(auto& v : vlist) bin_search(v.begin(), v.end(), randint(v.size())); }
            else bin_search(args.begin() + 2, args.end(), args[1], Strcmp{});
            break;  // random element as key
        }
        case SUB_MATCH: 
            if (args.size() < 3) out << "Need a SENTENCE and a WORD to match against\n";
            else {
                auto i = sub_match(args[1], args[2]);
                if (i != args[1].end()) out << "matched at position " << i - args[1].begin() << endl;
                else out << args[2] << " not found in " << args[1] << endl;
            }
            break;
        ...
        case INTERSECTION: {
            ...
            for (size_t i = 1; i < args.size(); ++i)
                s.insert(string(args[i]));
            print(intersection(s),out);
            break;
        }
        // generate all permutations of string
        case PERM: 
            if (args.size() < 3) out << "Need to specify word and permutation index\n";
            else {
            ...
            break;
        case ALLPERMS: 
            if (args.size() < 2) out << "Need to specify word\n";
            else print(allperms(args[1]),out); 
            break;
        // get all prime factors of number
        case FACTORIZE: 
            if (args.size() < 2) { load_data(vlist); time.restart(); for(auto& v : vlist) for (auto i : v) factorize(i); }
            else print(factorize(tol(args[1])),out);
            break;        
        // factor semi primes
        case FACTORIZE_ROUGH: 
            if (args.size() < 2) { load_data(vlist); time.restart(); for(auto& v : vlist) for (auto i : v) factorize_rough(i); }
            else print(factorize_rough(tol(args[2])),out);
            break;
        // substring finding
        case TOTIENT:
            if (args.size() < 2) out << "Totient takes 1 number and returns # of coprime ints under it\n";
            else out << totient(tol(args[1])) << endl;
            break;
        case GCD:
            if (args.size() < 3) out << "GCD takes 2 numbers\n";
            else out << gcd(tol(args[1]), tol(args[2])) << endl;
            break;
        case LCM:
            if (args.size() < 3) out << "LCM takes at least 2 numbers\n";
            else {
        ...
        case FIBONACCI:   // nth fibonacci term
            if (args.size() < 2) out << "Need to specify n\n";
            else out << fibonacci<unsigned long long>(tol(args[1])) << endl; 
            break;
        // by prime number theorem (PNT) count is ~ n/ln(n) and nth prime is ~ nln(n) -> gap is limited by n < p < 2n or n < p < 1.2n for n > 100
        case INF_PRIME: case NTH_PRIME: case IS_PRIME: case COUNT_PRIME: case NEXT_PRIME: case CLOSEST_PRIME: { // prime algorithms
            Sieve<> sieve {10000}; 
            switch (args.size()) {
                // infinite generation
                case 1: if (algo_index == INF_PRIME) while(true) out << sieve.next_prime() << " \n";
                ...
                case 2: {
                    ...
                    if (algo_index == NTH_PRIME) 
                        ...
                        if (sieve.is_prime(num)) out << num << " is prime\n"; 
                        else out << num << " is not prime\n";
        ...
        case BUB_SORT: case CNT_SORT: case INS_SORT: case HEAP_SORT: case LIN_SORT: case MER_SORT: case PAT_SORT:
        case QCK_SORT: case RDX_SORT: case TIM_SORT: case SORT: { // sorts
            load_data(vlist); 
            int bit_num {static_cast<int>(ceil(log2(range)))};
            time.restart();
            switch (algo_index) {
                case BUB_SORT: for(auto& v : vlist) bub_sort(v.begin(), v.end()); break;
                case CNT_SORT: for(auto& v : vlist) cnt_sort(v.begin(), v.end(), range+1); break;
                case INS_SORT: for(auto& v : vlist) ins_sort(v.begin(), v.end()); break;
                case HEAP_SORT: for(auto& v : vlist) heap_sort(v.begin(), v.end()); break;
                case LIN_SORT: for(auto& v : vlist) lin_sort(v.begin(), v.end()); break;
                case MER_SORT: for(auto& v : vlist) mer_sort(v.begin(), v.end()); break;
                case PAT_SORT: for(auto& v : vlist) pat_sort(v.begin(), v.end()); break;
                case QCK_SORT: for(auto& v : vlist) qck_sort(v.begin(), v.end()); break;
                case RDX_SORT: for(auto& v : vlist) rdx_sort(v.begin(), v.end(), bit_num); break; // # bits is log2 of maximum value
                case TIM_SORT: for(auto& v : vlist) tim_sort(v.begin(), v.end()); break;
                case SORT: for(auto& v : vlist) sort(v.begin(), v.end()); break;      
                default: break;
            }
        ...
        case LEVENSHTEIN: case SA_LC_SUBSTRING: case LC_SUBSTRING: case LC_SUBSEQ: case LC_SUBSEQ_LEN: {
            if (args.size() < 3) out << "Need 2 words to compare\n";
            else if (algo_index == LEVENSHTEIN) out << "levenshtein dist: " << levenshtein(args[1], args[2]) << endl;
            ...
            else if (algo_index == LC_SUBSTRING) out << "longest common substring: " << lc_substr(args[1], args[2]) << endl;
            else if (algo_index == LC_SUBSEQ) out << "longest common subsequence: " << lc_subseq(args[1], args[2]) << endl;
            else if (algo_index == LC_SUBSEQ_LEN) out << "longest common subsequence length: " << lc_subseq_len(args[1], args[2]) << endl;
        }
    ...
    out << "after: " << list_num << " lists with " << list_size << " elements: " << setw(10) << done_time <<  " us " 
        << setw(10) << done_time / 1000 << " ms  (" << ltrim(names_in_order[algo_index].second) << ")\n";
